...
void render() {
	...
	RayTracer rayTracer = RayTracer(sp, max_bounces, cutoff_weight, shadows);
...
void traceRayFunction(float x, float y) {
	...
	float tmin = 0.01f;
	RayTracer rt(sp, max_bounces, cutoff_weight, shadows);
	Vec3f pixel = rt.traceRay(ray, tmin, 1, 1, 1, hit);
}
void glrender() {
	canvas.initialize(sp, renderFunction, traceRayFunction);
}
